...
// start processing messages in parallel.
        var subscription = Task.Run(async () => { await Parallel.ForEachAsync(nats.SubscribeAsync<string>("greet", 
                cancellationToken: cts.Token), async (msg, _) => { Console.WriteLine($"Received {msg.Data}"); }); });
